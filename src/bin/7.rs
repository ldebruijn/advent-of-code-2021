...
fn main() {
    ...
    let won_game = play_bingo(&boards, &draws).expect("Game should be won");

    println!("Bingo! Draws {:?} winning board {:?}", won_game.draws, won_game.winning_board);

    let winning_board = boards.get(&won_game.winning_board).expect("Index should point to winning board");
    println!("Winning board {:?}", winning_board.items);
...
fn play_bingo(boards: &HashMap<usize, Bingo>, draws: &Vec<usize>) -> Option<WonGame> {
